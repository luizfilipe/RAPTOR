 ...
 */
	...
	void main(void) {
		...
		vec4 vertexPosition = vec4(position, 1.0);
			...
			//BoneTransform += bones[indices2[0]] * boneWeights2[0];
			//BoneTransform += bones[indices2[1]] * boneWeights2[1];
			//BoneTransform += bones[indices2[2]] * boneWeights2[2];
			//BoneTransform += bones[indices2[3]] * boneWeights2[3];
		
			vertexPosition = BoneTransform * vertexPosition;
			
		#endif
		
		v_worldPosition = world * vertexPosition;
	...
	float PCF(sampler2D depthMap, vec4 projCoords, vec2 randomDir, float depth)
	{
		...
		for(int i=0; i<kernelSize; i+=kernelStepSize)
		{
			...
			shadowSampler.x = ( depth < sampleDepth.x + shadowBias ) ? 1.0 : 0.6;
			shadowSampler.y = ( depth < sampleDepth.y + shadowBias ) ? 1.0 : 0.6;
			shadowSampler.z = ( depth < sampleDepth.z + shadowBias ) ? 1.0 : 0.6;
			shadowSampler.w = ( depth < sampleDepth.w + shadowBias ) ? 1.0 : 0.6;
	...
	float calculateShadowOcclusion( vec3 worldPosition, vec2 uv ) {
		...
		float shadowDepth = length( vec3(373.49224153337974, 357.85407167397125, -129.28294417250777).xyz - worldPosition );
	...
	void main() {
		...
		*/
		...
		lightObject light2;
		light2.position =  (vec3(373.49224153337974, 357.85407167397125, -129.28294417250777));
