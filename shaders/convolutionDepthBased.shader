 ...
 */
	...
	void main() {
		...
		vec2 imageCoordTmp = v_textureCoord - vec2(imageIncrement * 3.0);
		float sum = 0.0;
		
		float depth = texture2D(depthSampler, v_textureCoord).x;
		float invDepth = 1.0 - depth;
		
		for (int i = 0; i < 6; ++i) {
			sum += texture2D(image, imageCoordTmp).x * MSAA1X[i];
			imageCoordTmp += imageIncrement;
		}
		
		// sum += texture2D(image, imageCoordTmp).x * depth;
